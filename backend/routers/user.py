...
async def updateUser(user: updateUser = Depends(updateUser), token=Depends(validateJwtFromCookies)):
    ...
    hashed_password = hash_password(user.password)
    await collection.update_one({"Email": token}, {"$set": {"username": user.username, "password": hashed_password, "first_name": user.first_name, "last_name": user.last_name, "apps": user.apps, "role": user.role}})
